...
const Jobs = ({machines, connected, onConnect}) => {
  ...
  return (
    <div>
      ...
      <Button onClick={onRegister} isDisabled={connected} colorScheme={connected ? "green": "blue"} style={{marginTop: "10%", margin: "auto", display:"block"}}>{connected ? 'Connected' : 'Connect'}</Button>
      </div>
    </div>
  );
}
